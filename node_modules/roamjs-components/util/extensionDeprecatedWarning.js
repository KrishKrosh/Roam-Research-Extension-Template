...
const extensionDeprecatedWarning = ({ extensionId, reason, }) => tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    ...
    if (!donotShowAgainUid) {
        const blocks = window.roamAlphaAPI
            .q(`[:find (pull ?roamjs [:block/uid]) :where [?block :block/string ?contents] [(clojure.string/includes? ?contents  "https://roamjs.com/${extensionId}")] [?roamjs :block/children ?block]]`)
            .map(([{ uid }]) => uid);
